...
Alternativa cea mai des întâlnită în aplicațiile SPA este păstrarea token-ului în \texttt{localStorage} sau \texttt{sessionStorage}. Pentru Favigon, această variantă ar fi simplificat anumite detalii de implementare, dar ar fi crescut expunerea în cazul unor vulnerabilități de tip XSS, deoarece scripturile din pagină pot citi direct aceste zone de stocare. Folosirea cookie-urilor \textit{HTTP only} mută o parte importantă a protecției în browser și în pipeline-ul serverului. În schimb, această alegere cere mai multă atenție la configurarea politicilor CORS, a atributelor \texttt{SameSite} și a fluxurilor de refresh, deci nu este o soluție „gratuită”, ci un compromis orientat spre securitate operațională.
...
Valoarea practică a acestui strat se vede și în faptul că editorul și convertorul evoluează în ritmuri diferite. Editorul are nevoie de noțiuni precum selecție, istoric, clipboard, breakpoint-uri sau sincronizare între variante responsive, în timp ce convertorul are nevoie de clase, stiluri și structură DOM stabilă. Dacă ambele zone ar lucra direct pe HTML final, fiecare modificare a editorului ar risca să spargă exportul, iar fiecare regulă nouă din convertor ar risca să forțeze schimbări artificiale în UI. Reprezentarea intermediară reduce tocmai acest cuplaj și face proiectul mai ușor de extins pe termen lung.
...
Această diferență dintre „valid JSON” și „design utilizabil” este esențială pentru întreaga lucrare. Un model poate respecta schema și totuși să producă ierarhii greșite, dimensiuni nerealiste, noduri redundante sau combinații de stil dificil de exportat. Din acest motiv, JSON Schema este doar prima barieră de control, nu garanția finală a calității. Abia împreună cu validarea IR și cu regulile convertorului, schema devine parte dintr-un lanț de verificare care transformă ieșirea probabilistică a modelului într-un artefact suficient de robust pentru a intra în editor și apoi în export.
...
Din punct de vedere arhitectural, această combinație între model relațional și \texttt{jsonb} are și un avantaj de evoluție. Entitățile de bază, precum utilizatorul sau proiectul, au stabilitate mare și merită modelate explicit în tabele și relații. În schimb, structura internă a canvas-ului se schimbă mai des, mai ales când apar noi tipuri de noduri, proprietăți de layout sau reguli responsive. Dacă această structură ar fi fost normalizată excesiv în tabele separate, orice extindere a editorului ar fi cerut migrații mai frecvente și o complexitate mai mare la serializare și hidratare. Folosirea unui document JSON pentru design păstrează flexibilitatea exact acolo unde produsul evoluează cel mai repede.
...
Este important de subliniat că acest mecanism nu înseamnă că aplicația este „hostată de Cloudflare” în sensul clasic al termenului. Execuția frontend-ului, a API-ului și persistența datelor rămân în infrastructura controlată de proiect, iar Cloudflare Tunnel joacă rolul de strat de expunere și rutare securizată. Această distincție contează și în evaluarea tehnică a soluției: produsul rămâne responsabil pentru propriile procese, volume și configurări, iar tunelul doar simplifică accesul live și demonstrarea aplicației din exterior.
...
Mai important, această combinație reflectă și tipul de problemă rezolvată de Favigon. Aplicația nu este nici doar un CRUD clasic, nici doar un experiment AI, nici doar un editor vizual. Ea trebuie să susțină în același timp interacțiune bogată în browser, persistență de produs, integrare cu servicii externe și conversie controlată în cod. Din acest motiv, alegerea tehnologiilor a urmărit nu doar compatibilitatea lor individuală, ci și faptul că fiecare strat poate eșua, evolua sau fi testat relativ independent de celelalte. Tocmai această separare face posibilă extinderea ulterioară a proiectului fără rescrierea completă a bazei existente.
