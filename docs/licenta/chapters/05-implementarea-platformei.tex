...
Pe partea de client, aplicația este configurată prin \texttt{ApplicationConfig}, folosind \texttt{provideRouter}, \texttt{provideHttpClient} cu interceptori și \texttt{provideAnimations}. Această alegere este în linie cu stilul modern Angular bazat pe componente standalone. Din punct de vedere practic, ea reduce necesitatea unor module centrale voluminoase și face mai clară relația dintre rutare, infrastructura HTTP și comportamentul global al aplicației.

Un detaliu util este activarea \texttt{provideZoneChangeDetection} cu \texttt{eventCoalescing}. Pentru un produs cu editor vizual, acest tip de optimizare este relevant deoarece reduce numărul de actualizări declanșate de evenimente succesive și ajută la păstrarea unei interfețe mai fluide în scenarii de drag, selecție sau input repetat.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{3.4cm}YY}
    ...
    \midrule
    \texttt{/login} & autentificare și intrare în sistem & protejată de \texttt{loginEntryGuard}, pentru a evita revenirea inutilă după login \\
    \texttt{/reset-password} & recuperare acces și setare parolă nouă & rută publică, separată de fluxul clasic de autentificare \\
    \texttt{/project/:slug} & editorul canvas principal & protejată cu \texttt{authGuard} și cu \texttt{canDeactivate} pentru flush de persistență \\
    \texttt{/project/:slug/preview} & preview HTML al proiectului & necesită autentificare și reutilizează datele de proiect pentru randare separată \\
    \texttt{/explore} & descoperire de proiecte și utilizatori & accesibilă ca punct de intrare în componenta socială \\
    \texttt{/settings} & configurări de profil și cont & protejată pentru utilizatorul autentificat \\
    \texttt{/stars} & proiecte salvate de utilizator & afișează bookmark-urile proprii \\
    \texttt{/:username} & profilul utilizatorului curent & folosit și ca destinație implicită după încărcarea sesiunii \\
    \bottomrule
  \end{tabularx}
\end{table}
...
Componenta socială este implementată prin controllerele `ExploreController`, `ProjectsController` și `UsersController`, împreună cu serviciile și repository-urile aferente. Utilizatorii pot vizualiza proiecte populare sau recomandate, pot descoperi alți utilizatori, pot aprecia proiecte, le pot salva și le pot fork-ui.
...
Din perspectiva backend-ului, aplicația expune o suprafață REST suficient de largă încât să merite descrisă separat. În forma actuală, există șase controllere principale: \texttt{AccountController}, \texttt{ProjectsController}, \texttt{UsersController}, \texttt{ExploreController}, \texttt{AiDesignController} și \texttt{ConverterController}. Împreună, acestea acoperă autentificarea, datele de profil, proiectele, funcțiile sociale, generarea asistată de AI și exportul în cod.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{3.5cm}YY}
    ...
    \midrule
    \texttt{AccountController} & register, login, logout, refresh, resetare parolă, OAuth, 2FA & concentrează logica de sesiune și intrarea în sistem \\
    \texttt{ProjectsController} & CRUD proiecte, design, thumbnail, asset-uri, like, star, fork, view & este controllerul cel mai bogat funcțional din zona de produs \\
    \texttt{UsersController} & profil curent, profil public, follow/unfollow, linked accounts, imagine profil & leagă identitatea utilizatorului de componenta socială \\
    ...
    \texttt{AiDesignController} & generare AI clasică, streaming, pipeline pe faze & include atât răspuns sincron, cât și SSE incremental \\
    \texttt{ConverterController} & validare IR, generare HTML/React/Angular, fișiere multi-page & separă clar validarea de generarea finală \\
    \bottomrule
  \end{tabularx}
\end{table}
