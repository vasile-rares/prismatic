...
Această împărțire pare simplă, dar în practică aduce mai multe avantaje. Fiecare etapă are un contract mai clar. Fiecare etapă poate fi testată sau inspectată separat. Fiecare etapă poate fi oprită anticipat dacă utilizatorul vrea doar o schiță. În plus, dacă rezultatul unei etape este invalid, eroarea este mai ușor de localizat. Această abordare se aliniază cu observații din literatura de specialitate, unde descompunerea unei sarcini complexe în pași intermediari de raționament \cite{wei2022cot} sau în sub-sarcini modulare delegate unor prompturi dedicate \cite{khot2023decomposed} oferă un control și o fiabilitate mai bune decât o singură cerere directă. Fluxul general al acestei orchestrări este sintetizat în Figura~\ref{fig:pipeline-ai}.
...
Literatura recentă despre \textit{structured outputs} și \textit{constrained decoding} confirmă că această problemă merită tratată separat. Tehnicile de \textit{guided generation} demonstrează că ieșirea unui model poate fi constrânsă eficient să respecte o gramatică sau o schemă, garantând structura rezultatului cu un cost suplimentar redus \cite{willard2023guided}. La rândul său, \textit{JSONSchemaBench} arată că generarea de ieșiri conforme cu o schemă este o dimensiune tehnică distinctă de calitatea semantică a conținutului și propune evaluarea separată a conformității, eficienței și calității rezultatului \cite{geng2025jsonschemabench}. Această observație se potrivește foarte bine cu nevoile Favigon: pentru platformă nu este suficient ca răspunsul să fie valid JSON, ci trebuie să fie și suficient de coerent încât să poată fi editat și exportat mai departe.

În implementare, această limită apare concret atunci când un răspuns trece schema, dar produce o ierarhie slabă sau o structură greu de exportat. Din acest motiv, `AiPipelineService` nu se oprește la validarea structurală, ci aplică și reguli semantice și un pas de auto-reparare când rezultatul nu este acceptabil.
...
O componentă mai tehnică și mai interesantă este exportul responsive. Abordarea urmează principiile design-ului responsive introduse de Marcotte, bazate pe grile fluide, imagini flexibile și \textit{media queries} \cite{marcotte2010responsive}. În loc să dublez pur și simplu tot arborele pentru fiecare breakpoint, motorul produce artefactele principale și apoi diferențe CSS pentru breakpoints suplimentare. În testele existente se vede că sunt tratate cazuri precum noduri prezente doar pe anumite breakpoint-uri, schimbări de ordine pentru copii flex și diferențe de pseudo-clase sau keyframes.
